...
#define SENSORS_GP_PROXIMITY       (1<<ID_P)
#define SENSORS_GP_GROUP           (1<<ID_P)
...
static const struct sensor_t sSensorList[] = {
        { "KXSD9 3-axis Accelerometer",
                ...
                SENSOR_TYPE_ACCELEROMETER, 4.0f*9.81f, (4.0f*9.81f)/256.0f, 0.32f, { } },
	...
	{ "AK8973 Temperature sensor",
        ...
        { "GP2AP Proximity sensor",
                ...
                SENSOR_TYPE_PROXIMITY, 1.0f, 0.0f, 0.5f, { } },
        { "ISL29023 Light sensor",
                "Intersil",
                1, SENSORS_HANDLE_BASE+ID_L,
                SENSOR_TYPE_LIGHT, 10240.0f, 1.0f, 0.085f, { } },
};
...
const struct sensors_module_t HAL_MODULE_INFO_SYM = {
    .common = {
        ...
        .name = "AK8973A & GP2A & ILS Sensors Module",
...
#define GP_DEVICE_NAME      "/dev/proximity"
...
static uint32_t read_gp_sensors_state(int fd)
{
    int flags;
    uint32_t sensors = 0;
    // read the actual value of all sensors
    if (!ioctl(fd, SHARP_GP2AP_IOCTL_GET_ENABLED, &flags)) {
        if (flags)  sensors |= SENSORS_GP_PROXIMITY;
        else        sensors &= ~SENSORS_GP_PROXIMITY;
    }

    return sensors;
}

static int open_gp(struct sensors_control_context_t* dev)
{
    if (dev->gpd_fd < 0) {
        dev->gpd_fd = open(GP_DEVICE_NAME, O_RDONLY);
        LOGV("%s, fd=%d", __PRETTY_FUNCTION__, dev->gpd_fd);
        LOGE_IF(dev->gpd_fd<0, "Couldn't open %s (%s)",
                GP_DEVICE_NAME, strerror(errno));
        if (dev->gpd_fd >= 0) {
            dev->active_sensors &= ~SENSORS_GP_GROUP;
        }
    }
    return dev->gpd_fd;
}

static void close_gp(struct sensors_control_context_t* dev)
{
    if (dev->gpd_fd >= 0) {
        LOGV("%s, fd=%d", __PRETTY_FUNCTION__, dev->gpd_fd);
        close(dev->gpd_fd);
...
static int enable_disable_gp(struct sensors_control_context_t *dev,
                             uint32_t active, uint32_t sensors, uint32_t mask)
{
    int rc = 0;
    uint32_t now_active_gp_sensors;
    int fd = open_gp(dev);

    if (fd < 0) {
        LOGE("Couldn't open %s (%s)", GP_DEVICE_NAME, strerror(errno));
        return 0;
    }

    LOGV("(before) gp sensors = %08x, real = %08x",
         sensors, read_gp_sensors_state(fd));

    if (mask & SENSORS_GP_PROXIMITY) {
        int flags = (sensors & SENSORS_GP_PROXIMITY) ? 1 : 0;
        rc = ioctl(fd, SHARP_GP2AP_IOCTL_ENABLE, &flags);
        if (rc < 0)
            LOGE("SHARP_GP2AP_IOCTL_ENABLE error (%s)", strerror(errno));
    }

    now_active_gp_sensors = read_gp_sensors_state(fd);

    LOGV("(after) gp sensors = %08x, real = %08x",
         sensors, now_active_gp_sensors);

    return now_active_gp_sensors;
}
...
static uint32_t read_ls_sensors_state(int fd)
{
    ...
    uint32_t sensors = 0;
    // read the actual value of all sensors
    if (!ioctl(fd, KIONIX_ISL_IOCTL_GET_ENABLED, &flags)) {
                             ...
                             uint32_t active, uint32_t sensors, uint32_t mask)
{
    ...
    if (mask & SENSORS_LIGHT) {
        ...
        rc = ioctl(fd, KIONIX_ISL_IOCTL_ENABLE, &flags);
        if (rc < 0)
            LOGE("KIONIX_ISL_IOCTL_ENABLE error (%s)", strerror(errno));
    }
        ...
        int handle, int enabled)
{
    ...
    if (changed) {
        ...
        dev->active_sensors =
            ...
            enable_disable_gp(dev,
                              active & SENSORS_GP_GROUP,
                              new_sensors & SENSORS_GP_GROUP,
                              changed & SENSORS_GP_GROUP) |
...
static uint32_t data__poll_process_gp_abs(struct sensors_data_context_t *dev,
                                           int fd __attribute__((unused)),
                                          struct input_event *event)
{
    uint32_t new_sensors = 0;
    if (event->type == EV_ABS) {
        ...
        if (event->code == EVENT_TYPE_PROXIMITY) {
            new_sensors |= SENSORS_GP_PROXIMITY;
            dev->sensors[ID_P].distance = (event->value == 1.0f ? 0.0f : 1.0f);
        }
    }
    return new_sensors;
}
